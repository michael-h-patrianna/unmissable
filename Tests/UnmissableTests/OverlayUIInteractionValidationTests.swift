...
final class OverlayUIInteractionValidationTests: XCTestCase {
  ...
  func testOverlayContentViewButtonCallbacks() throws {
  ...
  func testOverlayContentViewWithDifferentEventTypes() throws {
    ...
    let testCases: [(event: Event, description: String)] = [
      ...
      (
        TestUtilities.createTestEvent(
          title: "Very Long Meeting Title That Should Be Handled Gracefully"), "Long title event"
      ),
    ]
  ...
  func testOverlayContentViewSnoozeOptions() throws {
  ...
  func testOverlayContentViewErrorHandling() throws {
  ...
  func testOverlayContentViewStateConsistency() throws {
  ...
  func testOverlayContentViewWithRealEventData() throws {
    ...
    let realWorldEvent = Event(
      ...
      calendarId: "primary",
      links: [
        URL(string: "https://meet.google.com/abc-def-ghi")!,
        URL(string: "https://docs.google.com/document/d/agenda")!,
      ],
      provider: .meet
    )
