...
final class StartedMeetingsTests: DatabaseTestCase {
  ...
  func testFetchStartedMeetings() async throws {
    ...
    let startedMeeting = Event(
      ...
      links: [URL(string: "https://meet.google.com/test-link")!]
    )
    ...
    let upcomingIds = upcomingMeetings.map { $0.id }
  ...
  func testGoogleMeetLinkDetection() async throws {
    ...
    let now = Date()
    let meetingWithGoogleMeet = Event(
    ...
    let meeting = startedMeetings.first!
  ...
  func testJoinButtonTenMinuteWindow() async throws {
    ...
    let tooEarlyMeeting = Event(
      ...
      endDate: now.addingTimeInterval(1800),  // 30 minutes from now
      calendarId: "test-calendar",
      links: [URL(string: "https://meet.google.com/too-early")!]
    )

    let justInTimeMeeting = Event(
      ...
      endDate: now.addingTimeInterval(1800),  // 30 minutes from now
      calendarId: "test-calendar",
      links: [URL(string: "https://meet.google.com/just-in-time")!]
    )
    ...
    let startedMeeting = Event(
      ...
      endDate: now.addingTimeInterval(1200),  // 20 minutes from now
      calendarId: "test-calendar",
      links: [URL(string: "https://meet.google.com/started")!]
    )
    ...
    let endedMeeting = Event(
      ...
      calendarId: "test-calendar",
      links: [URL(string: "https://meet.google.com/ended")!]
    )
